...
namespace MyHook
{
    ...
    // sarcheck.dll 入口点地址 （眠れる花は春をまつ。）
    DWORD g_AlphaRomOffset = 0x00A410A5;     // 模式A = 绝对地址，模式B = 未使用
    LPVOID g_TargetAddr = nullptr;

    // 入口点 39 字节特征码
    BYTE g_Signature[39] = {
        0x51,0xB9,0x06,0x00,0x00,0x00,0x85,0xC9,0x74,0x28,0x01,0xC8,0xD3,0xE0,0x05,
        0x45,0x45,0x53,0x04,0x50,0xE8,0x00,0x00,0x00,0x00,0x83,0x04,0x24,0x16,0x8B,
        0x04,0x24,0x83,0xC0,0x09,0xC7,0x80,0xCA,0xFF
    };
    ...
    // 区域检测HOOK
    BYTE g_RegionSig[32] = {
     0x0F,0x85,0xA9,0x00,0x00,0x00,0x6A,0x5B,0x33,0xFF,0x68,0xD8,0x0E,0x7A,0x00,
     0x8D,0x74,0x24,0x54,0xC7,0x44,0x24,0x6C,0x07,0x00,0x00,0x00,0x89,0x7C,0x24,
     0x68,0x66
    };

    BYTE g_RegionPatch[6] = { 0xE9,0x8B,0x02,0x00,0x00,0x90 };  // jmp 00465781; nop
    ...
    // DVD检测HOOK 
    BYTE g_DVDSig[11] = {
    0x32,0xC0,0xEB,0x5A,0x6A,0x12,0x68,0x64,0xD3,0x79,0x00
    };

    BYTE g_DVDPatch[2] = { 0xB0,0x01 };  // mov al,1

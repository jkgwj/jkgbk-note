...
namespace MyHook
{
    ...
    // sarcheck.dll 入口点地址 （Rewrite Harvest festa!）
    DWORD g_AlphaRomOffset = 0x04FB2551;     // 模式A = 绝对地址，模式B = 未使用
    LPVOID g_TargetAddr = nullptr;

    // 入口点 70 字节特征码
    BYTE g_Signature[70] = {
        0x83,0xEC,0x04,0x50,0x53,0xE8,0x01,0x00,0x00,0x00,0xCC,
        0x58,0x89,0xC3,0x40,0x2D,0x00,0x50,0x27,0x00,0x2D,0x7F,
        0xCF,0x09,0x10,0x05,0x74,0xCF,0x09,0x10,0x80,0x3B,0xCC,
        0x75,0x19,0xC6,0x03,0x00,0xBB,0x00,0x10,0x00,0x00,0x68,
        0xAF,0x4C,0x85,0x66,0x68,0xF0,0x95,0xD4,0x2D,0x53,0x50,
    ...
    // 区域检测HOOK
    BYTE g_RegionSig[67] = {
     0x0F,0x85,0x92,0x00,0x00,0x00,0x6A,0x5B,0x33,0xFF,0xBB,0x07,0x00,0x00,0x00,
     0x68,0x68,0x22,0x7D,0x00,0x8D,0x74,0x24,0x5C,0x89,0x5C,0x24,0x74,0x89,0x7C,
     0x24,0x70,0x66,0x89,0x7C,0x24,0x60,0xE8,0xF4,0x02,0xFA,0xFF,0x89,0xBC,0x24,
     0xD0,0x00,0x00,0x00,0x6A,0x0D,0x68,0x20,0x23,0x7D,0x00,0x8D,0x74,0x24,0x40,
     0x89,0x5C,0x24,0x58,0x89,0x7C,0x24
    };

    BYTE g_RegionPatch[6] = { 0xE9,0x84,0x02,0x00,0x00,0x90 };  // jmp 0046116B; nop
    ...
    // DVD检测HOOK 
    BYTE g_DVDSig[71] = {
    0x0F,0x84,0xE5,0x02,0x00,0x00,0xBB,0x07,0x00,0x00,0x00,0x89,0x5C,0x24,0x54,
    0x89,0x7C,0x24,0x50,0x66,0x89,0x7C,0x24,0x40,0x8D,0x4C,0x24,0x3C,0x55,0x51,
    0x8D,0x74,0x24,0x60,0x89,0xBC,0x24,0x14,0x01,0x00,0x00,0xE8,0x0F,0x08,0x00,
    0x00,0x68,0x38,0xD7,0x7C,0x00,0x50,0x8D,0xB4,0x24,0xA0,0x00,0x00,0x00,0xC6,
    0x84,0x24,0x1C,0x01,0x00,0x00,0x01,0xE8,0xB5,0xAA,0xFA
    };

    BYTE g_DVDPatch[6] = { 0xE9,0x42,0x03,0x00,0x00,0x90 };  // jmp 0045766A; nop

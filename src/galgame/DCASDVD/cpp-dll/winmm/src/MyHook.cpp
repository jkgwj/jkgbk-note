...
namespace MyHook
{
    ...
    HookEntry g_DVDCheckHook = {
        0x00014E13,        // 特征码偏移 (00414E13 - 00400000)
        g_DVDSig, 2,       // 特征码及长度
        { 0x00014E13 },    // 补丁偏移
        { nullptr },       // 不额外校验原始指令
        { g_DVDPatch },    // 补丁数据
    ...
    // Hook 序列号验证
    BYTE g_SerialSig[6] = { 0x83, 0xEC, 0x14, 0x83, 0xC9, 0xFF };
    BYTE g_SerialPatch[6] = { 0xB8, 0x00, 0x00, 0x00, 0x00, 0xC3 };

    HookEntry g_SerialCheckHook = {
        0x00024800,        // 特征码偏移 (00424800 - 00400000)
        g_SerialSig, 6,    // 特征码及长度
        { 0x00024800 },    // 补丁偏移
        { nullptr },       // 不额外校验原始指令
        { g_SerialPatch }, // 补丁数据
        { 6 },             // 补丁长度
    ...
    {
        // 获取 dcasdvd.exe 基址
        g_BaseAddr = (DWORD)::GetModuleHandleW(L"dcasdvd.exe");
        if (!g_BaseAddr)
            return 1;
